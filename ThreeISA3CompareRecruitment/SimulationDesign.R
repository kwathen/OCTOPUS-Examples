...
SetupSimulations <- function( cTrialDesign, nQtyReps,
                              ...
                              vISAStartTimes = NULL,
                              dMaxPatsPerSitePerMonth = 0.5,
                              vMaxNumberOfSites = seq( 30, 100, 10),
                              nDesign = 1)
{
    ...
    #
    vPatsPerMonthPerSite1   <- c( seq( dMaxPatsPerSitePerMonth/6, dMaxPatsPerSitePerMonth, length =5), rep( dMaxPatsPerSitePerMonth, 40))
    vQtyOfSitesPlat         <- vMaxNumberOfSites[ 1 ] #c(3,     8,  15,   35,  50, 70)
    ...
    vMaxNumberOfSites <- vMaxNumberOfSites[ -1 ]  # Remove the first number of sites since it is already added as secenario 1 

    for( iScen in 1:length( vMaxNumberOfSites ) )   # Loop over the scenarios in vMaxNumberOfSites
    {
        cScen          <- cScen1

        vPatsPerMonthPerSite1   <- c( seq( dMaxPatsPerSitePerMonth/6, dMaxPatsPerSitePerMonth, length =5), rep( dMaxPatsPerSitePerMonth, 40))
        vQtyOfSitesPlat         <- vMaxNumberOfSites[ iScen ] #c(3,     8,  15,   35,  50, 70)
        vQtyOfPatsPerMonth1     <- vPatsPerMonthPerSite1 * vQtyOfSitesPlat
